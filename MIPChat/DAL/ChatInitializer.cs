...
namespace MIPChat.DAL
{
    class ChatInitializer : DropCreateDatabaseAlways<ChatDBContext>
    {
        protected override void Seed(ChatDBContext db)
        {
            ...
            Message m1 = new Message() { MessageId = Guid.NewGuid(), Content = "I am banan", AuthorId = u1.UserId, TheTimeOfSending = DateTime.Now };
            Message m2 = new Message() { MessageId = Guid.NewGuid(), Content = "You are banan", AuthorId = u2.UserId, TheTimeOfSending = DateTime.Now };
            Message m3 = new Message() { MessageId = Guid.NewGuid(), Content = "Oy shit", AuthorId = u1.UserId, TheTimeOfSending = DateTime.Now };
            Message m4 = new Message() { MessageId = Guid.NewGuid(), Content = "Here we go again", AuthorId = u2.UserId, TheTimeOfSending = DateTime.Now };
            db.Messages.AddRange(new List<Message> { m1, m2, m3, m4 });
            /*Images for chat*/
            ChatModel cm1 = new ChatModel() { ChatId = Guid.NewGuid(), Name = "About life", Users = new List<User>() { u1, u2 }, Messages = new List<Message>() { m1, m2, m3, m4 } };
            ChatModel cm2 = new ChatModel() { ChatId = Guid.NewGuid(), Name = "Empty", Users = new List<User>() { u1, u3, u4 }, Messages = new List<Message>() };
